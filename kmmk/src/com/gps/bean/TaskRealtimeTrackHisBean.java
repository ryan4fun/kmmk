...
public class TaskRealtimeTrackHisBean extends AbstractBean {
	static Logger logger = Logger.getLogger(TaskRealtimeTrackHisBean.class);
	...
	public TaskRealtimeTrackHisBean() {
	}

	public TaskRealtimeTrackHisBean(HttpServletRequest request) {
		super(request);
	}

	public List<TaskRealtimeTrackHis> getList(){
		try {
			Criteria crit = HibernateUtil.getSession().createCriteria(TaskRealtimeTrackHis.class);
			...
			List<TaskRealtimeTrackHis> list = crit.list();
	...
	public TaskRealtimeTrackHis findById(){
		if(id!=null && id.intValue()>0)
			return getServiceLocator().getTaskRealtimeTrackHisService().findById(id);
		else
			return null;
	}
