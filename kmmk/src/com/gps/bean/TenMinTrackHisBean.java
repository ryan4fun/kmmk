...
public class TenMinTrackHisBean extends AbstractBean {
	static Logger logger = Logger.getLogger(TenMinTrackHisBean.class);
	...
	public TenMinTrackHisBean() {
	}

	public TenMinTrackHisBean(HttpServletRequest request) {
		super(request);
	}

	public List<TenMinTrackHis> getList(){
		try {
			Criteria crit = HibernateUtil.getSession().createCriteria(TenMinTrackHis.class);
			...
			List<TenMinTrackHis> list = crit.list();
	...
	public TenMinTrackHis findById(){
		if(id!=null && id.intValue()>0)
			return getServiceLocator().getTenMinTrackHisService().findById(id);
		else
			return null;
	}
