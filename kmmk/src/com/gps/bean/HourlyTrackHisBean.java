...
public class HourlyTrackHisBean extends AbstractBean {
	static Logger logger = Logger.getLogger(HourlyTrackHisBean.class);
	...
	public HourlyTrackHisBean() {
	}

	public HourlyTrackHisBean(HttpServletRequest request) {
		super(request);
	}

	public List<HourlyTrackHis> getList(){
		try {
			Criteria crit = HibernateUtil.getSession().createCriteria(HourlyTrackHis.class);
			...
			List<HourlyTrackHis> list = crit.list();
	...
	public HourlyTrackHis findById(){
		if(id!=null && id.intValue()>0)
			return getServiceLocator().getHourlyTrackHisService().findById(id);
		else
			return null;
	}
