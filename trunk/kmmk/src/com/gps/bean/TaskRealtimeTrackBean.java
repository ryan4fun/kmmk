...
public class TaskRealtimeTrackBean extends AbstractBean {
	static Logger logger = Logger.getLogger(TaskRealtimeTrackBean.class);
	...
	public TaskRealtimeTrackBean() {
	}

	public TaskRealtimeTrackBean(HttpServletRequest request) {
		super(request);
	}

	public List<TaskRealtimeTrack> getList(){
		try {
			Criteria crit = HibernateUtil.getSession().createCriteria(TaskRealtimeTrack.class);
			...
			List<TaskRealtimeTrack> list = crit.list();
	...
	public TaskRealtimeTrack findById(){
		if(id!=null && id.intValue()>0)
			return getServiceLocator().getTaskRealtimeTrackService().findById(id);
		else
			return null;
	}
