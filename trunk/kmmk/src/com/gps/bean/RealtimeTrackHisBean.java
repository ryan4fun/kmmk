...
public class RealtimeTrackHisBean extends AbstractBean {
	static Logger logger = Logger.getLogger(RealtimeTrackHisBean.class);
	...
	public RealtimeTrackHisBean() {
	}

	public RealtimeTrackHisBean(HttpServletRequest request) {
		super(request);
	}

	public List<RealtimeTrackHis> getList(){
		try {
			Criteria crit = HibernateUtil.getSession().createCriteria(RealtimeTrackHis.class);
			...
			List<RealtimeTrackHis> list = crit.list();
	...
	public RealtimeTrackHis findById(){
		if(id!=null && id.intValue()>0)
			return getServiceLocator().getRealtimeTrackHisService().findById(id);
		else
			return null;
	}
